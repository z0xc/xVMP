...
// clang-format on
uint8_t gv_data_seg[SEG_SIZE] = {0};
...
#endif
uint64_t
unpack_addr(uint64_t address, int size) {
...
#endif
    void
    pack_store_addr(uint64_t address, uint64_t value, int size) {
  uint8_t *ptr = (uint8_t *)address;
...
uint64_t
get_value_with_size(uint8_t value_size, uint8_t value_type) {
...
#endif
    void
    data_seg_clean(int return_value_off) {
